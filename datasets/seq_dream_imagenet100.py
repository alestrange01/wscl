...
from datasets.seq_imagenet100 import MyImagenet100, SequentialImagenet100, MyImagenet100Sal, SequentialImagenet100Sal
...
class DreamDatasetSal(MyImagenet100Sal):

    DREAM_CLASS_LABELS = []
    DREAM_CLASS_NAMES = []

    def __init__(self, root_1: str, root_2: str, train: bool = True, transform: Optional[nn.Module] = None, target_transform: Optional[nn.Module] = None,
                 subset: float = 1, labels = list(range(SequentialImagenet100Sal.N_CLASSES_PER_TASK)) + [None] * SequentialImagenet100Sal.N_CLASSES_PER_TASK * (SequentialImagenet100Sal.N_TASKS-1)) -> None:
        ...
        d2_name = os.path.split(root_2)[-1]
        if d2_name == 'AUXImageNet100':
            dataset_2 = MyImagenet100Sal(root_2, train, transform, target_transform, subset, primary=False)
        else:
            raise ValueError(f'Dataset: {d2_name} not supported')
        ...
        if len(DreamDatasetSal.DREAM_CLASS_LABELS) == 0:
            if num_classes_2 > num_classes_1:
                DreamDatasetSal.DREAM_CLASS_LABELS = np.random.choice(num_classes_2, len(labels), replace=False)
            else:
                DreamDatasetSal.DREAM_CLASS_LABELS = np.arange(num_classes_2)
            #save dream_classes names
            DreamDatasetSal.DREAM_CLASS_NAMES = [dataset_2.classes[l] for l in DreamDatasetSal.DREAM_CLASS_LABELS]
        ...
        #fill dataset with dream data
        dream_labels = [DreamDatasetSal.DREAM_CLASS_LABELS[i] if labels[i] == None else None for i in range(len(labels))]
        index_mask = [t in dream_labels for t in dataset_2.targets]
        data_2 = dataset_2.data[index_mask]
        targets_2 = np.array(dataset_2.targets)[index_mask]
...
class DreamImagenetSal(SequentialImagenet100Sal):
    NAME = 'dream-seq-img100sal'
    ...
    def get_data_loaders(self):
        ...
        train_dataset = DreamDatasetSal(self.args.dataset_path + 'ImageNet100', self.args.dataset_path + self.args.dataset_2, train=True, transform=transform, labels = list(self.pos_to_label.values()))
        test_dataset = DreamDatasetSal(self.args.dataset_path + 'ImageNet100', self.args.dataset_path + self.args.dataset_2, train=False, transform=test_transform, labels = list(self.pos_to_label.values()))
